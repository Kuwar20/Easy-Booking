...
(0:07) Project UI -> Sign In -> Register -> (0:16) Registration Validation -> Project Main Page -> Sign Out -> Sign In Check -> My Hotels Page Check -> (1:00) Add Hotel -> Check Added Hotel in My Hotels Page -> Update Hotel -> Verify Added Hotel on Main Page -> Check Search Feature -> Check Sorting Among Saved Hotels -> Check Hotel Details from Search -> Try Booking Hotel without Selecting Dates -> Return to Hotel Page to Select Date -> Confirm Booking Test with Test Card Details for Stripe -> Go Back to Main Page and Check Booked Hotel -> Sign Out to Test Non-User Booking -> Sign In to Book (Show Instead of Book Now Button) -> End
```
